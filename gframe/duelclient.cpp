...
void DuelClient::HandleSTOCPacketLan(unsigned char* data, size_t len) {
	...
	case STOC_SRVPRO_ROOMLIST: {
		...
		hosts_srvpro.clear();
		unsigned short count = BufferIO::Read<uint16_t>(pdata);
		char temp1[64];
		char temp2[128];
		wchar_t roomname[32];
		wchar_t player1[64];
		...
		for(unsigned short i = 0; i < count; ++i) {
			...
			unsigned char room_status = BufferIO::Read<uint8_t>(pdata);
			char room_duel_count = BufferIO::Read<int8_t>(pdata);
			char room_turn_count = BufferIO::Read<int8_t>(pdata);

			memcpy(temp2, pdata, 128);
			pdata += 128;
			BufferIO::DecodeUTF8(temp2, player1);

			char player1_score = BufferIO::Read<int8_t>(pdata);
			int player1_lp = BufferIO::Read<int32_t>(pdata);

			memcpy(temp2, pdata, 128);
			pdata += 128;
			BufferIO::DecodeUTF8(temp2, player2);

			char player2_score = BufferIO::Read<int8_t>(pdata);
			int player2_lp = BufferIO::Read<int32_t>(pdata);
...
bool DuelClient::ClientAnalyze(unsigned char* msg, size_t len) {
	...
	case MSG_SELECT_CARD: {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (l & LOCATION_OVERLAY)
				...
				pcard = mainGame->dField.CreateCard();
	...
	case MSG_CONFIRM_CARDS: {
		...
		if (!skip_panel && panel_confirm.size() && mainGame->dInfo.player_type != 7 && !auto_watch_mode) {
			...
			myswprintf(textBuffer, dataManager.GetSysString(208), static_cast<int>(panel_confirm.size()));
			mainGame->wCardSelect->setText(textBuffer);
	...
	case MSG_TOSS_COIN: {
		...
		int count = BufferIO::Read<uint8_t>(pbuf);
		std::wstring result;
		result += dataManager.GetSysString(1623);
		for (int i = 0; i < count; ++i) {
			uint8_t res = BufferIO::Read<uint8_t>(pbuf);
			result += L'[';
			result += dataManager.GetSysString(res ? 60 : 61);
			result += L']';
		}
		if(mainGame->dInfo.isReplaySkiping)
			return true;
		...
		mainGame->gMutex.lock();
		mainGame->AddLog(result.c_str());
		mainGame->stACMessage->setText(result.c_str());
		mainGame->PopupElement(mainGame->wACMessage, 20);
		mainGame->gMutex.unlock();
		mainGame->WaitFrameSignal(40);
		return true;
	...
	case MSG_TOSS_DICE: {
		...
		int count = BufferIO::Read<uint8_t>(pbuf);
		std::wstring result;
		result += dataManager.GetSysString(1624);
		for (int i = 0; i < count; ++i) {
			uint8_t res = BufferIO::Read<uint8_t>(pbuf);
			result += L'[';
			result += (L'0' + res);
...
void DuelClient::SendResponse() {
	...
	case MSG_SELECT_COUNTER: {
		...
		for (auto* card : mainGame->dField.limbo_temp)
			mainGame->dField.DestroyCard(card);
...
unsigned int DuelClient::LookupHost(char *host) {
	return ResolveHostName(host, nullptr);
}
...
HostResult DuelClient::ParseHost(char *hostname) {
	...
	if(inet_pton(AF_INET, hostname, &address) == 1) {
		result.host = ntohl(address.s_addr);
		result.port = 7911;
		return result;
	}
