...
void TagDuel::StartDuel(DuelPlayer* dp) {
	...
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[0].main.size() * 10);
	BufferIO::Write<uint16_t>(pbuf, extra_size[0] * 5);
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[0].side.size());
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].main.size() * 10);
	BufferIO::Write<uint16_t>(pbuf, extra_size[1] * 5);
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].side.size());
#else
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[0].main.size() * 10);
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[0].extra.size() * 5);
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[0].side.size());
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].main.size() * 10);
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].extra.size() * 5);
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].side.size());
#endif
...
int TagDuel::Analyze(unsigned char* msgbuffer, unsigned int len) {
	...
	while (pbuf - msgbuffer < (int)len) {
		...
		switch (engType) {
		case MSG_RESET_TIME: {
			player = BufferIO::Read<int8_t>(pbuf);
			short time = BufferIO::Read<int16_t>(pbuf);
			if(host_info.time_limit) {
				if(time)
		...
		case MSG_UPDATE_CARD: {
			auto controller = BufferIO::Read<uint8_t>(pbuf);
			auto location = BufferIO::Read<uint8_t>(pbuf);
			auto sequence = BufferIO::Read<uint8_t>(pbuf);
			auto clen = BufferIO::Read<int32_t>(pbuf);
			auto query_flag = BufferIO::Read<uint32_t>(pbuf);
			pbuf += (clen - 8);
		...
		case MSG_NEW_TURN: {
			int r_player = BufferIO::Read<int8_t>(pbuf);
#ifdef YGOPRO_SERVER_MODE
