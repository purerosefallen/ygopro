...
void TagDuel::StartDuel(DuelPlayer* dp) {
	...
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].main.size());
	BufferIO::Write<uint16_t>(pbuf, extra_size[1]);
	...
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[0].side.size());
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].main.size());
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].extra.size());
	BufferIO::Write<uint16_t>(pbuf, (short)pdeck[2].side.size());
#endif
...
#endif //YGOPRO_SERVER_MODE
{
	...
	while(qlen < len) {
		...
		if (clen <= LEN_HEADER)
			continue;
		NetServer::StripRevealFlag(qbuf, 8);
		qbuf += clen - 4;
	}
#ifdef YGOPRO_SERVER_MODE
...
void TagDuel::RefreshSingle(int player, int location, int sequence, int flag) {
	...
	if (location & LOCATION_ONFIELD)
		NetServer::StripRevealFlag(qbuf, 12);
	NetServer::SendBufferToPlayer(players[pid], STOC_GAME_MSG, query_buffer, len + 4);
	NetServer::ReSendToPlayer(players[pid + 1]);
#ifdef YGOPRO_SERVER_MODE
	NetServer::ReSendToPlayer(replay_recorder);
