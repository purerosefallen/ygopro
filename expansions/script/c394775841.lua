...
function c394775841.initial_effect(c)
	--link summon
	aux.AddLinkProcedure(c,aux.FilterBoolFunction(Card.IsLinkType,TYPE_RITUAL),3)
	c:EnableReviveLimit()
	--special summon
	...
	e2:SetDescription(aux.Stringid(394775841,1))
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_ATTACK_ANNOUNCE)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCondition(c394775841.condition)
	e2:SetCost(c394775841.cost)
	e2:SetOperation(c394775841.operation)
	c:RegisterEffect(e2)
end
...
function c394775841.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetAttacker():IsControler(1-tp)
end
function c394775841.cfilter(c)
	return c:IsSetCard(0xb4) and c:IsAbleToGraveAsCost()
end
function c394775841.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c394775841.cfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,c394775841.cfilter,1,1,REASON_COST)
end
function c394775841.operation(e,tp,eg,ep,ev,re,r,rp)
