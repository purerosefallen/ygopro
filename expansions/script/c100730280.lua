...
function c100730280.con(e,tp)
	...
	return aux.SpeedDuelAtMainPhaseCondition(e,tp)
		...
		and Duel.IsExistingMatchingCard(c100730280.filter1,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil)
...
function c100730280.skill(e,tp,c)
	...
	if chk==0 then return g:CheckSubGroup(c100730280.fselect,2,3) end
	local rg=g:SelectSubGroup(tp,c100730280.fselect,false,2,3)
	Duel.SendtoGrave(rg,REASON_COST)
	local ct=rg:GetCount()
	local g2=Duel.SelectMatchingCard(tp,c100730280.filter1,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil)
	...
	if Duel.SpecialSummonStep(sc,0,tp,tp,false,false,POS_FACEUP_DEFENSE) then
		...
		local sg=Duel.GetMatchingGroup(c100730280.filter2,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_GRAVE,0,nil)
		...
		e3:SetDescription(aux.Stringid(100730280,1))
		e3:SetCategory(CATEGORY_DESTROY)
		e3:SetType(EFFECT_TYPE_IGNITION)
		e3:SetProperty(EFFECT_FLAG_CARD_TARGET)
		e3:SetRange(LOCATION_MZONE)
		e3:SetTarget(c100730280.destg)
		e3:SetOperation(c100730280.desop)
...
function c100730280.filter2(c)
	return c:IsCode(60999392) or c:IsCode(96384007) or c:IsCode(23782705) or c:IsCode(22666164) and not c:IsForbidden()
end
function c100730280.desfilter(c,g)
	return c:IsFaceup() and c:IsCode(60999392) or c:IsCode(96384007) or c:IsCode(23782705) or c:IsCode(22666164) and g:IsContains(c)
end
function c100730280.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local g=e:GetHandler():GetEquipGroup()
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c100730280.desfilter,tp,LOCATION_ONFIELD,0,1,nil,g)
		and Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectTarget(tp,c100730280.desfilter,tp,LOCATION_ONFIELD,0,1,1,nil,g)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g2=Duel.SelectTarget(tp,Card.IsFaceup,tp,0,LOCATION_ONFIELD,1,1,nil)
	g1:Merge(g2)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g1,2,0,0)
end
function c100730280.desop(e,tp,eg,ep,ev,re,r,rp)
