...
function c100730295.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsFaceup() and c:IsSetCard(0x1045)
end
function c100730295.con(e,tp)
	tp=e:GetLabelObject():GetOwner()
	return aux.SpeedDuelAtMainPhaseCondition(e,tp)
		and Duel.IsExistingMatchingCard(c100730295.filter,tp,LOCATION_MZONE,0,1,nil)
end
function c100730295.skill(e,tp)
	tp=e:GetLabelObject():GetOwner()
	Duel.Hint(HINT_CARD,1-tp,100730295)
	local g=Duel.GetMatchingGroup(c100730295.filter,tp,LOCATION_MZONE,0,nil)
