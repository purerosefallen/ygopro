...
function c100730192.skill(e,tp)
	...
	e1:SetCountLimit(1)
	e1:SetCondition(c100730192.con)
	e1:SetReset(RESET_PHASE+PHASE_END,count)
	e1:SetOperation(c100730192.op)
	Duel.RegisterEffect(e1,tp)
	e:Reset()
end
function c100730192.filter(c)
	return c:IsRace(RACE_FAIRY) and c:IsFaceup() and c:IsStatus(STATUS_SUMMON_TURN)
end

function c100730192.op(e,tp,eg,ep,ev,re,r,rp)
...
function c100730192.con(e,tp,eg,ep,ev,re,r,rp)
	if count==2 then return Duel.GetTurnCount()==2 end
	return Duel.IsExistingMatchingCard(c100730192.filter,tp,LOCATION_MZONE,0,1,nil)and Duel.IsExistingMatchingCard(Card.IsFacedown,tp,LOCATION_SZONE,0,2,nil) and Duel.GetMZoneCount(tp)>1
end
