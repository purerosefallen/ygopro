...
function c100730196.monfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsLevelAbove(5)
end
function c100730196.filter(c,g)
	if not c100730196.monfilter(c) then return false end
	local tc=g:GetFirst()
	while tc do
...
function c100730196.skill(e,tp,eg,ep,ev,re,r,rp)
	tp=e:GetLabelObject():GetOwner()
	local g=Group.CreateGroup()
	if not Duel.IsExistingMatchingCard(c100730196.filter,tp,LOCATION_DECK,0,3,nil,g) then
		Duel.Hint(HINT_MESSAGE,tp,aux.Stringid(100730196,0))
		e:Reset()
		return
	end
	...
	if (not gA:IsExists(c100730196.monfilter,1,nil)) and (gB:IsExists(c100730196.monfilter,1,nil)) then
		local tc=gB:GetFirst()
		local sp=nil
		while tc do
			if c100730196.monfilter(tc) and sp==nil then
				sp=tc
				gBFinal:Merge(gA)
				gA:Clear()
				gA:AddCard(tc)
